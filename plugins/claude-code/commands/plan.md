...
description as `intent` first. It defaults to the current project; pass
`project` only to draft for a different one. It returns conventions, the
closest KB entries, an example plan, and the drafter prompt to work from.
Then produce a plan in the SerenEdge JSON schema:
...
Call the `submit_plan` MCP tool with your `plan` and `apply: false`. It
defaults to the current project; pass `project` only to plan a different one.
It returns validation errors (if any - fix them and resubmit) or a diff:
which tasks are new, which match an existing task and would change, and
which are unchanged. Show this report to the Architect in plain language and
iterate on the plan until they are happy.

Once they approve, call `submit_plan` again with `apply: true` (this needs
`plan.write`). It defaults to the current project; pass `project` only to
plan a different one. It creates or updates the tasks, wires up `depends_on`,
